...
/***/ "./resources/js/components/photo.edit.js":
/*!***********************************************!*\
  !*** ./resources/js/components/photo.edit.js ***!
  \***********************************************/
/***/ (function() {
...
document.addEventListener('alpine:init', function () {
  window.Alpine.data('cropper', function () {
    return {
      $store: undefined,
      photoId: '',
      subjectId: '',
      ...
      width: '',
      init: function init() {},
      cropContainerClick: function cropContainerClick() {},
      onCropReload: function onCropReload() {
        var e = document.querySelector('.crop-container > div');
        _this.subjectId = e.getAttribute('id');
        _this.dataSpread = e.getAttribute('data-spread');
        _this.dataLayout = e.getAttribute('data-layout');
        _this.dataLayoutPhoto = e.getAttribute('data-photo');

        _this.cropContainerPreload();
      },
      //////////////////////
      getCoverWidth: function getCoverWidth() {
        window.Livewire.emit('getCoverWidth', 'djdjdjdj'); // return  document.getElementById('photo-cover').innerHTML
      },
      onPhotoDragStart: function onPhotoDragStart(a) {
        this.subjectImage = a.target.getAttribute('src');
        this.photoId = a.target.getAttribute('data-id'); // this.fireChoseEditingPage()
      },
      onPhotoClick: function onPhotoClick(a) {
      ...
      onPhotoDrop: function onPhotoDrop(e) {
      ...
      subjectOnEnter: function subjectOnEnter(e) {
        e.preventDefault();
      },
      fireChoseEditingPage: function fireChoseEditingPage() {
        if (!document.querySelectorAll(".edit").length) {
          Toast.fire({
            icon: 'warning',
            title: 'Сначала нужно выбрать страницу'
          });
          return false;
        }
      },
      onPhotoRemove: function onPhotoRemove(imgId) {
        window.Livewire.emit('onPhotoRemove', imgId);
      },
      onDeleteCropper: function onDeleteCropper(a) {
      ...
      onResetCropper: function onResetCropper(a) {
        var parent = a.target.parentNode;
        var img = a.target.parentNode.getElementsByTagName('img')[0];
      ...
      cropContainerPreload: function cropContainerPreload() {
        var that = this;
        var cropper = new Croppic(this.subjectId, {
          ...
          loaderHtml: '',
          modal: true,
          onBeforeImgCrop: function onBeforeImgCrop() {
            var resource = document.getElementById(this.id);
            var imgStyle = document.querySelector('.cropImgWrapper img');
...
/***/ "./resources/js/store/photo.editor.js":
/*!********************************************!*\
  !*** ./resources/js/store/photo.editor.js ***!
  \********************************************/
/***/ (() => {
...
document.addEventListener('alpine:init', function () {
  Alpine.store('editor', {
    init: function init() {
      ...
      this.oddClass = '';
      this.evenClass = '';
      this.uploadImageActive = ''; // this.covwerWidth = this.getCoverWidth()

      window.addEventListener('unsetActiveSpread', function () {
        _this.oddClass = '';
        _this.evenClass = '';
      });
    },
    getCoverWidth: function getCoverWidth() {
      ...
      window.Livewire.emit('getCoverWidth', document.getElementById('photo-cover').offsetWidth); // return  document.getElementById('photo-cover').innerHTML
    },
    activatePhoto: function activatePhoto() {
      window.Livewire.emit('activePhoto');
    },
    activateOdd: function activateOdd() {
      if (this.oddClass === 'edit') return false;
      this.evenClass = '';
      this.oddClass = 'edit';
      window.Livewire.emit('activeOddSpreadEvent');
    },
    activateEven: function activateEven() {
      if (this.evenClass === 'edit') return false;
      this.evenClass = 'edit';
      this.oddClass = '';
...
__webpack_require__(/*! ./store/photo.editor */ "./resources/js/store/photo.editor.js");

__webpack_require__(/*! ./components/photo.edit */ "./resources/js/components/photo.edit.js");
