    ...
    @foreach($this->printout->size->cover as $coverType)
        ...
        @foreach($coverType->cover->item_materials  as $material)
            ...
            <li class="flex flex-col sm:flex-row gap-8 sm:gap-16">
                ...
                <label class="block" wire:click="onSetCoverAttributes({{$coverType->cover->id}},{{$material->id}})">
                    ...
                    @if($spread && $spread->cover_material_id === $material->id && $coverType->cover->id === $spread->cover_type_id)
