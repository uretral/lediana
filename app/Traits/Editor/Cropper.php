...
trait Cropper
{
    ...
    public function onImageSaveEvent($layout_photo_id, $spread_nr, $layout_id, $photo_id, $html, $cropData)
    {
        $this->printout->photos()->updateOrCreate(
            ['printout_id' => $this->printout_id, 'spread_nr' => $spread_nr, 'layout_id' => $layout_id, 'layout_photo_id' => $layout_photo_id],
            ['photo_id' => $photo_id, 'html' => $html, 'cropper' => $cropData]
        );
    ...
    public function getPhotoHtml(PrintoutSpread $spread, LayoutPhoto $photo)
    {
        $res = $this->printout->photos()
            ...
            ->first();
        if($res && $res->html) {
            $res->html = str_replace('<img', '<img class="photo-cropped" data-id="' . $res->photo_id . '"', $res->html);
        }
        return $res;
    }
